...
namespace Prime31
{
	...
	public class SpriteLightKit : MonoBehaviour
	{
		...
		public string textureName = "SpriteLightKit RT";
...
//				var rtHeight = Mathf.RoundToInt( _spriteLightCamera.pixelHeight * rtSizeMultiplier );
				var rtWidth = Mathf.RoundToInt( mainCamera.pixelWidth * rtSizeMultiplier ) ;
				var rtHeight = Mathf.RoundToInt( mainCamera.pixelHeight * rtSizeMultiplier );

				if( rtWidth == 0 || rtHeight == 0 )
				{
					Debug.LogWarning( "RT height or width rounded to 0. Defaulting to the camera pixelWidth" );
					rtWidth = mainCamera.pixelWidth;
					rtHeight = mainCamera.pixelWidth;
				}
